...
struct CustomError: ErrorInfo, Error, LocalizedError, Identifiable {
    var error: NSError?
    var id = UUID()
    ...
    init(error: AsyncSwiftConnectError) {
        if let nsError = error.error {
            self.init(error: nsError)
        } else if let errorCode = error.errorCode, let code = Int(errorCode) {
            self.init(responseCode: code)
        } else {
            self.init(unknowError: error.errorInfo ?? "n/a")
        }
    }
