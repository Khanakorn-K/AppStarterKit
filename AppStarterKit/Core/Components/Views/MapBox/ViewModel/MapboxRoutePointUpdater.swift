...
class MapboxRoutePointUpdater<RoutePin: View, RoutePinResponseModel: Codable>: ObservableObject {
    ...
    //@Published var routePointDataSource: [RoutePointDataSource] = []
...
//            .store(in: &cancellable)
    ...
    private func getRoutePoint(bbox: String) async throws -> RoutePinResponseModel {
        let url = "https://layer.jertam.com/\(isDev)jertammap/ows?service=WFS&version=1.0.0&request=GetFeature&typename=\(isDev)jertammap:walkruntrailpoint&outputFormat=application/json&srsname=EPSG:4326&bbox=\(bbox)"
        
        return try await MapboxAPICreator(url: (urlForRoutePin?(bbox) ?? url).url).build()
    }
